...
const int RAM_RX_CHECK_LEN = sizeof(ram_rx_checks) / sizeof(ram_rx_checks[0]);
...
const safety_hooks ram_hooks = {
  ...
  .fwd = ram_fwd_hook,
  .addr_check = ram_rx_checks,
  .addr_check_len = sizeof(ram_rx_checks) / sizeof(ram_rx_checks[0]),
};